<template>
  <div>
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':item.checked=='1'}" @click="editQuantity('check',item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editQuantity('minus',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="editQuantity('plus',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':checkAllCount==0}" @click="checkOut()">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal v-bind:mdShow="confirmShow" @close="closeModal">
        <p slot="message">
            Are you sure to delect the item?
        </p>
        <div slot="btnGroup">
            <a href="javascrip:;" class="btn btn--m" @click="confirmDel">Confirm</a>
            <a href="javascrip:;" class="btn btn--m" @click="confirmShow=false">Cancel</a>
        </div>
    </modal>
...
<script>
  ...
  import {currency} from "./../util/currency.js"
    export default{
        ...
        // },
        computed:{
          checkAll(){   //替代原来data里定义的checkAll, 这里是一个实时属性的计算
            return this.checkAllCount == this.cartList.length;

          },
          checkAllCount(){  //这里的checkAllCount是一个计算属性而非方法
            var i = 0;
          ...
          totalPrice(){
            ...
            this.cartList.forEach((item)=>{
              if(item.checked == '1'){
                tPrice += parseFloat(item.salePrice)*parseInt(item.productNum);
              }
            });
            return tPrice;
          }
        ...
        methods: {
          ...
          confirmDel(){
            axios.post('/users/cartDel',{
              productId_query: this.delProductid
            }).then((response)=>{
          ...
          editQuantity(query, item){
            if(query == "plus"){
              ...
              else{item.productNum--;}
            }
            else{
              item.checked = item.checked=='1'?'0':'1'
            }
            axios.post('/users/editQuantity',{
              productId_query: item.productId,
              productNum_query: item.productNum,
              check_query: item.checked
            }).then((response)=>{
              if(response.data.status=='0'){
          ...
          toggleCheckAll(){
            ...
            let checkAllFlag = !this.checkAll;    //这里是一个实时属性的计算
            this.cartList.forEach((item)=>{
              item.checked = !checkAllFlag=='0'?'1':'0';
            });
            axios.post('/users/editCheckAll',{
              checkAll_query: checkAllFlag
            }).then((response)=>{
