<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFilter" :class="{'check': checkIndex==index}" @click="checkIndex=index;seletedAddressId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.addressId,index)"><i>Set default</i></a>
                    </div>
            ...
            <div class="next-btn-wrap">
              ...
              <router-link class="btn btn--m btn--red" href="javascript:;" v-bind:to="{path:'orderConfirm',query:{'addressId_query':seletedAddressId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal v-bind:mdShow="confirmShow" @close="closeModal">
        <p slot="message">
            Are you sure to delect the Address?
        </p>
        <div slot="btnGroup">
      ...
      methods: {
            ...
            if(response.data.status=='0'){
                ...
                this.addressList.forEach((item, index)=>{
                  if(item.isDefault == true){
                    ...
                    this.seletedAddressId = item.addressId;
                  }
                });
              }
              else{
        ...
        confirmDel(){
          axios.post('/users/addressDel',{
              addressId_query: this.delAddressId
            }).then((response)=>{
              if(response.data.status=='0'){
