            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.salePrice*item.productNum | currencyFormat('$')}}</div>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div class="price-count-wrap">
          <div class="price-count">
            <ul>
              ...
              <li>
                <span>Shipping:</span>
                <span>{{shipping | currencyFormat('$')}}</span>
              </li>
              <li>
                <span>Discount:</span>
                <span>{{discount | currencyFormat('$')}}</span>
              </li>
              <li>
                <span>Tax:</span>
              ...
              <li class="order-total-price">
                ...
                <span>{{orderTotal | currencyFormat('$')}}</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      methods: {
        ...
        payMent(){
          axios.post('/users/payMent',{
            addressId_query: this.$route.query.addressId_query,
            orderTotalPrice_query: this.orderTotal
          }).then((response)=>{
            if(response.data.status=='0'){
              this.$router.push({
                path: '/orderSuccess?orderId='+response.data.results.orderId
              });
            }
            else{
